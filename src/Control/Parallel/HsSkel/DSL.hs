...
data Stream dim f d where
    StGen     :: (NFData o) => (i -> (Maybe (o, i))) -> i -> Stream Z f o
    ...
    StStop    :: (DIM dim) => dim -> Skel f (c, i) c -> c -> Skel f c Bool -> Stream dim f i -> Stream dim f i
...
stStop :: (DIM dim) => Skel f (c, i) c -> c -> Skel f c Bool -> Stream dim f i -> Stream dim f i
...
class StGenSupport f fun i o where
...
instance (NFData o) => StGenSupport f (i -> (Maybe (o, i))) i o where
    stGen = StGen

instance (NFData o) => StGenSupport f (i -> (o, i)) i o where
